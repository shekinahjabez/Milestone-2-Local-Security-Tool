#!/usr/bin/env python3
"""
NetworkTraffic Analyzer - Build Script (PortableApps Edition)
==============================================================
Compiles the application into a portable executable using PyInstaller.

The binary is placed in App/NetworkTrafficAnalyzer/ so the launcher
scripts can find it. All build artifacts stay inside this directory
and can be cleaned with --clean.
...
APP_NAME = "NetworkTrafficAnalyzer"
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
MAIN_SCRIPT = os.path.join(SCRIPT_DIR, "main.py")
SPEC_FILE = os.path.join(SCRIPT_DIR, f"{APP_NAME}.spec")

# The compiled binary goes into this same directory
DIST_DIR = SCRIPT_DIR
...
def check_scapy():
    ...
    try:
        import scapy
        print(f"[OK] Scapy {scapy.VERSION} found.")
        return True
    except ImportError:
...
def build(onedir=False):
    """Run PyInstaller to produce the portable executable."""
    ...
    # PyInstaller needs a temp dist dir; we move the binary after
    ...
    # Move binary from dist/ to App/NetworkTrafficAnalyzer/
    ...
    # Clean up temp dist/ and build/ directories
    ...
    print(f"{'=' * 60}")
    print(f"\nTo run: use NetworkTrafficAnalyzerPortable.sh (Linux)")
    print(f"or NetworkTrafficAnalyzerPortable.bat (Windows)")
    print(f"from the NetworkTrafficAnalyzerPortable/ root folder.")
